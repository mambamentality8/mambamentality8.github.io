...
1、生成密钥证书 下边命令生成<font color="red" size=5>密钥证书</font>，采用RSA 算法每个<font color="red" size=5>证书包含公钥和私钥</font>

```
keytool -genkeypair -alias changgou -keyalg RSA -keypass changgou -keystore changgou.jks -storepass changgou 
```

<font color="red" size=5>私钥采用了RSA算法</font>
...
请求<font color="red" size=5>认证</font>服务获取授权码：

<font color="red" size=5>/oauth/authorize这个路由是Oauth内部提供的</font>
...
<font color="red" size="5">客户端ID和客户端密钥就是相当于微信的账号密码</font>

<font color="red" size="5">我要拿着我的微信账号和密码去微信那边申请授权码</font>

<font color="red" size="5">相当于我扫码的过程</font>
...
<font color="red" size="5">微信给我弹出来一个授权页面问我是否同意使用我的微信账号密码去登陆第三方的资源</font>

<font color="red" size="5">防止坏人从不同的终端登陆盗用而不经过我的手直接访问了第三方资源盗取我的money</font>
...
<font color="red" size=5>/oauth/token这个是这个路由是Oauth内部提供的</font>
...
![img](第9章SpringSecurity整合Oauth2.assets/1558219035988.png)
...
基本上所有微服务都是资源服务，这里我们在用户管理服务上配置授权控制，当配置了授权控制后如要访问其它信息则必须提供令牌。
...
<font color="red" size=5>有坑 账号密码不对   这里只校验了密码没有校验账号</font>
...
<font color="red" size=5>坑 账号密码都为itheima</font>

<font color="red" size=5>测试的时候去掉user-oauth里面的判断</font>
